...
class NoteServiceTest {
    ...
    fun `delete existed comment`() {
        val owner = User(1, "Owner")
        val noteService = NoteService()
        noteService.addNote("132jkl123ljk", "12hjk32", owner.id)
        noteService.createComment(1, owner.id, owner.id, "1123132")
        val test = noteService.deleteComment(1, 1, owner.id, owner.id)
        assertTrue(test)
    }

    @Test
    fun `delete not existed comment`() {
        val owner = User(1, "Owner")
        val noteService = NoteService()
        noteService.addNote("132jkl123ljk", "12hjk32", owner.id)
        noteService.createComment(1, owner.id, owner.id, "1123132")
        val test = noteService.deleteComment(1312312, 1, owner.id, owner.id)
        assertFalse(test)
    }
    ...
    fun `edit existed comment`() {
        ...
        noteService.addNote("132jkl123ljk", "12hjk32", owner.id)
        noteService.createComment(1, owner.id, owner.id, "1123132")
        val test = noteService.editComment(1, owner.id, 1, owner.id, "zzzzzzzzzzz")
        assertTrue(test)
    }

    @Test
    fun `edit not existed comment`() {
        val owner = User(1, "Owner")
        val noteService = NoteService()
        noteService.addNote("132jkl123ljk", "12hjk32", owner.id)
        noteService.createComment(1, owner.id, owner.id, "1123132")
        val test = noteService.editComment(1312312, owner.id, 1, owner.id, "zzzzzzzzzzz")
        assertFalse(test)
    }

    @Test
    fun `restore not deleted comment`() {
        val owner = User(1, "Owner")
        val noteService = NoteService()
        noteService.addNote("132jkl123ljk", "12hjk32", owner.id)
        noteService.createComment(1, owner.id, owner.id, "1123132")
        val test = noteService.restoreComment(1, owner.id, 1)
        assertFalse(test)
    }
}
